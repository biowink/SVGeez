module SVGeez::Output::IOSLayerTree
	class Renderer
		...
		def run
			...
			fo_h.puts "+ (CALayer *)layerForKey:(NSString *)key;"
			fo_h.puts "@end"

			fo_m.puts "@implementation SVGeez"
			fo_m.puts "+ (CALayer *)layerForKey:(NSString *)key {"
			xxsvgs.keys.each do |key|
			...
			xxsvgs.each do |key, svg|
				fo_m.puts "+ (CALayer *)_layerForKey_#{key} {"
				fo_m.puts svg
				fo_m.puts "\treturn svg;"
				fo_m.puts "}"
				fo_m.puts
			end
		...
		def dump_svg_with_children element, writer
			...
			when "svg"
				output = SVGeez::Output::IOSLayerTree::SVG.new(actual_element)
			when "circle"
				output = SVGeez::Output::IOSLayerTree::Circle.new(actual_element)
			when "ellipse"
				output = SVGeez::Output::IOSLayerTree::Ellipse.new(actual_element)
			when "g"
				output = SVGeez::Output::IOSLayerTree::Container.new(actual_element)
			when "path"
				output = SVGeez::Output::IOSLayerTree::Path.new(actual_element)
			when "polygon"
				output = SVGeez::Output::IOSLayerTree::Polygon.new(actual_element)
			when "polyline"
				output = SVGeez::Output::IOSLayerTree::Polyline.new(actual_element)
			when "rect"
				output = SVGeez::Output::IOSLayerTree::Rect.new(actual_element)
			end
